...
public:
	...
	Projector(PointInputIt points_begin, PointInputIt points_end,
			const double& max_width, const double& max_height,
...
template<typename PointInputIt>
Sphere::Projector::Projector(PointInputIt points_begin, PointInputIt points_end,
		const double& max_width, const double& max_height,
		const double& padding) :
		padding_(padding) {
	auto min_lat = std::min_element(points_begin, points_end,
			[](const Sphere::Point& p1, const Sphere::Point& p2) {
   ...
   if (coef_lon == 0 && coef_lat == 0){
  	   zoom_coef_ = 0;
     }
     else if (coef_lon == 0){
  	   zoom_coef_ = (max_height - 2 * padding) / coef_lat;
     }
     else if (coef_lat == 0){
  	   zoom_coef_ = (max_width - 2 * padding) / coef_lon;
     }
     else
  	   {zoom_coef_ = std::min((max_width - 2 * padding) / coef_lon, (max_height - 2 * padding) / coef_lat);}
