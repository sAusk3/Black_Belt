...
static vector<Svg::Color> ParseColours(const Json::Node& json) {
	const auto& array = json.AsArray();
...
static NeighboursDicts BuildCoordNeighboursDicts(const unordered_map<string, Sphere::Point>& stops_dict,
                                                 const Descriptions::BusesDict& buses_dict) {
...
static map<std::string, Svg::Color> ChooseBusColors(
...
RenderSettings ParseRenderSettings(const Json::Dict& json) {
	...
	result.palette = ParseColours(json.at("color_palette"));
...
for(const auto& [bus_name, bus_ptr] : buses_dict_) {
	...
	Svg::Polyline line;
	line.SetStrokeColor(buses_colours_.at(bus_name))
...
void MapRenderer::RenderBusLabels(Svg::Document& svg) const {
	for (const auto& [bus_name, bus_ptr] : buses_dict_) {
		const auto& stops = bus_ptr->stops;
		if(!stops.empty()){
		   const auto& color = buses_colours_.at(bus_name);
...
MapRenderer::MapRenderer(const Descriptions::StopsDict& stops_,
		const Descriptions::BusesDict& buses_,
		const Json::Dict& render_settings_json_) :
...
CoordsCompressor::CoordsCompressor(const std::unordered_map<std::string, Sphere::Point>& stops_dict){
	for (const auto& [_, coord] : stops_dict) {
