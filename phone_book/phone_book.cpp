...
AddressBook DeserializeAddressBook(std::istream& input) {
	...
	for (auto& item : *contacts.mutable_contact()) {
		...
		if (item.has_birthday()) {
			current.birthday = Date { item.birthday().year(),
...
AddressBook::ContactRange AddressBook::FindByNamePrefix(
		std::string_view name_prefix) const {
	auto range = std::equal_range(contacts_.cbegin(), contacts_.cend(),
			name_prefix, NameComparator());
