...
public:
	...
	Data ComputeSum(IndexSegment segment) const {
		return this->TraverseWithQuery(root_.get(), segment,
				ComputeSumVisitor { });
	}

	void AddBulkOperation(IndexSegment segment,
			const BulkOperation& operation) {
		this->TraverseWithQuery(root_.get(), segment, AddBulkOperationVisitor {
				operation });
	}
...
private:
	...
	static Node* Build(IndexSegment segment) {
		if (segment.empty()) {
			...
			return new Node { .left = nullptr, .right = nullptr, .segment =
					segment, };
		} else {
			const size_t middle = segment.left + segment.length() / 2;
			return new Node { .left = Build( { segment.left, middle }), .right =
					Build( { middle, segment.right }), .segment = segment, };
		}
	}

	template<typename Visitor>
	static typename Visitor::ResultType TraverseWithQuery(Node* node,
			IndexSegment query_segment, Visitor visitor) {
		if (!node || !AreSegmentsIntersected(node->segment, query_segment)) {
			...
			if (query_segment.Contains(node->segment)) {
				...
				if constexpr (is_void_v<typename Visitor::ResultType>) {
					...
					return visitor.ProcessPartial(node, query_segment,
							TraverseWithQuery(node->left, query_segment,
									visitor),
							TraverseWithQuery(node->right, query_segment,
									visitor));
				}
			}
		}
	...
	public:
		...
		Data ProcessPartial(Node*, IndexSegment, const Data& left_result,
...
public:
	...
	BudgetManager() :
			tree_(
					new SummingSegmentTree<MoneyState, BulkLinearUpdater>(
							DAY_COUNT)) {
	}
...
struct ComputeIncomeRequest: ReadRequest<double> {
	...
	{
		return manager.GetTree()->ComputeSum(
				MakeDateSegment(date_from, date_to)).ComputeIncome();
	}
	...
	void Process(BudgetManager& manager) const override
	{
		...
		const MoneyState daily_change = SIGN == 1 ? MoneyState { .earned =
				daily_value } :
													MoneyState { .spent =
															daily_value };
		manager.GetTree()->AddBulkOperation(date_segment,
				BulkLinearUpdater(manager.GetTree(), BulkMoneyAdder {
						daily_change }));
	}
...
struct PayTaxRequest: ModifyRequest {
	...
	void Process(BudgetManager& manager) const override
	{
		manager.GetTree()->AddBulkOperation(MakeDateSegment(date_from, date_to),
